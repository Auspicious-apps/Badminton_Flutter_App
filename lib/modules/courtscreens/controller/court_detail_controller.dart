...
// Fetch current user and friend requests
  ...
  Future<void> getFriendRequest() async {
    ...
    try {
      final response = await _apiRepository.getRequest(
          {"status": "friends-requests", "search": searchQuery.value?.trim()});
      if (response != null) {
    ...
    try {
      ...
      if (response != null) {
        ...
        if (userdata.value?.data?.courts?.isNotEmpty ?? false) {
          ...
          updateCourtSelection(firstCourt.sId ?? '',
              game: firstCourt.games ?? 'Padel');
        }
      }
    } catch (e) {
      Get.snackbar('Error', e.toString(), snackPosition: SnackPosition.TOP);
    ...
    try {
      ...
      try {
        ...
        if (await canLaunchUrlString(fallbackUrl)) {
          ...
          Get.snackbar('Error', 'Could not open map (fallback)',
              snackPosition: SnackPosition.TOP, duration: Duration(seconds: 3));
        }
      } catch (e) {
        print('Error launching fallback map: $e');
        Get.snackbar('Error', 'Could not open map (fallback): $e',
            snackPosition: SnackPosition.TOP, duration: Duration(seconds: 3));
      }
    }
  }
  ...
  void updateCourtSelection(String courtId,
      {String? rate, AvailableSlots? timeSlot, String? game}) {
    ...
    List<AvailableSlots> timeSlotsCopy =
        List<AvailableSlots>.from(selection.timeSlots);
  ...
  void showFriendsBottomSheet(BuildContext context, int slotIndex) {
    ...
    if (slotIndex == 0) {
      Get.snackbar('Info', 'This slot is reserved for you',
          snackPosition: SnackPosition.TOP);
      return;
    }
    ...
    showModalBottomSheet(
      context: context,
      isScrollControlled:
          true, // Allow the bottom sheet to adjust height for keyboard
      builder: (context) {
        return Padding(
          ...
          child: Column(
            ...
            children: [
              ...
              Obx(
                () => friendsdata.value.data?.friends?.length == 0
                    ? SizedBox(
                        height: 50,
                        child: Center(
                    ...
                    : ListView.builder(
                        ...
                        itemBuilder: (context, index) {
                          final friend =
                              friendsdata.value.data?.friends?[index];
                          bool friendExists = selectedFriends.any(
                                  (selected) => selected?.sId == friend?.sId) ||
